...
import { MDSPolicy, Geography, VehicleEvent, Device } from '@mds-core/mds-types'
...
async function main(): Promise<(ComplianceEngineResult | undefined)[]> {
  ...
  const policies = (await readJson(args.policies)) as MDSPolicy[]
  ...
  // emit results
  return Promise.all(
    supersedingPolicies.map(async (policy: MDSPolicy) => {
