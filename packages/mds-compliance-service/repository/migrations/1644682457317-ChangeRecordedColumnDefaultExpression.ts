...
export class ChangeRecordedColumnDefaultExpression1644682457317 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "compliance_snapshots" ALTER COLUMN "recorded" SET DEFAULT mds_current_timestamp_ms()`
    )
    await queryRunner.query(
      `ALTER TABLE "compliance_snapshot_failures" ALTER COLUMN "recorded" SET DEFAULT mds_current_timestamp_ms()`
    )
    await queryRunner.query(
      `ALTER TABLE "compliance_violations" ALTER COLUMN "recorded" SET DEFAULT mds_current_timestamp_ms()`
    )
  }
