...
export class CreateComplianceSnapshotFailuresTable1634323054883 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE "compliance_snapshot_failures" ("recorded" bigint NOT NULL DEFAULT (extract(epoch from now()) * 1000)::bigint, "id" BIGINT GENERATED ALWAYS AS IDENTITY, "compliance_snapshot_id" uuid NOT NULL, "timestamp" bigint NOT NULL, CONSTRAINT "compliance_snapshot_failures_pkey" PRIMARY KEY ("compliance_snapshot_id"))`
    )
    await queryRunner.query(
