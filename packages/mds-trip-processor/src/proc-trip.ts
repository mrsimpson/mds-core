...
function calcTimeIntervals(telemetry: { [x: string]: { [x: string]: { timestamp: any } } }, start_time: any) {
  /*
...
async function process_trip(
  ...
  */
  ...
  let trip_data = {
    ...
    violation_count: 0,
    max_violation_dist: 0,
    min_violoation_dist: 0,
    avg_violation_dist: 0,
    telemetry: Array()
  }
  ...
  if (trip_telemetry && trip_telemetry.length > 0) {
    console.log('Parsing telemtry data')
  ...
  let dist_measure: any = calcDistance(trip_data.telemetry, trip_start_info.gps)
  let total_distance = dist_measure.totalDist
  trip_data.distance = total_distance
  let dist_array = dist_measure.points
  trip_data.violation_count = dist_measure.points.length
  trip_data.max_violation_dist = Math.min(...dist_array)
  trip_data.min_violoation_dist = Math.max(...dist_array)
  if (dist_array.length > 0) {
    let add = (a: number, b: number) => a + b
    let sum = dist_array.reduce(add)
    trip_data.avg_violation_dist = sum / dist_array.length
  }
